...
class Jacobi(Poly):
	...
	def __init__(self, module, degree, x, alpha, beta):
		...
		if module == 'tensorflow':
			...
			recurrence = lambda p1, p2, n, x: ((2*n+alpha+beta+1)*tf.multiply((2*n+alpha+beta+2)*(2*n+alpha+beta)*x+alpha**2-beta**2, p1) - 2*(n+alpha)*(n+beta)*(2*n+alpha+beta+2)*p2)/(2*(n+1)*(n+alpha+beta+1)*(2*n+alpha+beta))
		elif module == 'pytorch':
			initial = [lambda x: torch.ones_like(x), lambda x: 0.5*(alpha+beta+2)*x+0.5*(alpha-beta)]
			recurrence = lambda p1, p2, n, x: ((2*n+alpha+beta+1)*((2*n+alpha+beta+2)*(2*n+alpha+beta)*x+alpha**2-beta**2)*p1 - 2*(n+alpha)*(n+beta)*(2*n+alpha+beta+2)*p2)/(2*(n+1)*(n+alpha+beta+1)*(2*n+alpha+beta))
